...
function befuellen(ticket) {
    
    let termin = document.getElementById("termin");
    let bezeichnung = document.getElementById("bezeichnung");
    termin.innerHTML =  tickets.termin; 
    bezeichnung.innerHTML = tickets.bezeichnung;
    return `${ticket.bezeichnung}: ${ticket.termin} <br/>`
}

let ticketContainer = document.getElementById("ticket-container"); //Testcontainer
    tickets.forEach((ticket) => {
        let ergebnis = befuellen(ticket);
        ticketContainer.innerHTML += ergebnis;
})
